...
class FtCtrl extends FilmControl
{
...
class FtClient
{
    ...
    private FilmUtility $utility;

    public function __construct(FilmUtility $utility, array $options)
    {
    ...
    public function get_utility(): FilmUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FilmContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FilmContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FilmConfig::make_feature($name);
    ...
    private static function buildUrl(FilmSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FilmError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FilmResponse($fetched);
        }
    ...
    private function fail(FilmContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FilmContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
