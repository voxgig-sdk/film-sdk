...
module FilmUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FilmResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FilmResponse.new(fetched)
    else
