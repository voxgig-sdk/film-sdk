...
//     (\`new FilmSDK(...)\` / \`FilmSDK.test(...)\` become
//     \`FilmSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"film":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FilmSDK(...)\` and any
// \`FilmSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FilmSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FilmSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
