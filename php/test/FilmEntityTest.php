...
class FilmEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FILM_TEST_FILM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function film_basic_setup($extra)
{
    ...
    $client = FilmSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("FILM_TEST_FILM_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FILM_TEST_FILM_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["FILM_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new FilmSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["FILM_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
