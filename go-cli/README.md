...
boru-driven command-line client **and** interactive REPL for the Film
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/film-cli)
make build
...
./film-cli list film
./film-cli load 1 film            # {id:1} shorthand
./film-cli load '{id:1}' film       # explicit match map

# 5. Override the API base URL for a single call
FILM_BASE=https://api.example.com ./film-cli list film
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/film-cli
   ```
...
```sh
./film-cli load 1 film          # scalar shorthand for {id:1}
./film-cli load '{id:1}' film     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `film>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/film-cli`. |
...
(`./film-cli <expr>`) and interactively (the REPL), and expressions compose the
