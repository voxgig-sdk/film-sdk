// Command Film-cli is a CLI + REPL for the Film SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FILM_APIKEY (api key), FILM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FILM_APIKEY carries the API key and
	// FILM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FILM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FILM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFilmSDK(opts)
