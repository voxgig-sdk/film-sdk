...
class FilmEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FILM_TEST_FILM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def film_basic_setup(extra)
  ...
  entid_env_raw = ENV["FILM_TEST_FILM_ENTID"]
  ...
  if env["FILM_TEST_LIVE"] == "TRUE"
    ...
    client = FilmSDK.new(Helpers.to_map(merged_opts))
  end
