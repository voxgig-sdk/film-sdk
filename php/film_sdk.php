...
class FilmSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FilmHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FilmHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FilmFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FilmFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FilmUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FilmHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FilmHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FilmHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FilmHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FilmError("graphql_error",
                "FilmSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
