...
MCP server exposing the Film SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `film_list` | `entity`, optional `query` map | First page of records as JSON |
| `film_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `film_list` and
`film_load` tools become available in new sessions.
