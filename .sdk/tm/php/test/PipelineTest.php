...
{
    private static function utility(): FilmUtility
    {
        return new FilmUtility();
    }
    ...
    private static function ctx(array $over = []): FilmContext
    {
        ...
        $ctx->op = new FilmOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FilmError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FilmOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FilmMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FilmOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FilmMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FilmOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FilmMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FilmSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FilmMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx([
            'spec' => new FilmSpec(['step' => 's']),
            'response' => new FilmResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FilmResult([]),
        ]);
        [, $err] = FilmMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FilmSpec(['step' => 's']),
            'response' => new FilmResponse(self::resp(200, ['v' => 1])),
            'result' => new FilmResult([]),
        ]);
        [, $err] = FilmMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FilmSpec(['step' => 's']),
            'response' => new FilmResponse(self::resp(200, ['v' => 2])),
            'result' => new FilmResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FilmMakeResponse::call($ctx);
    ...
    {
        $preset = new FilmResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FilmSpec([]),
            'response' => new FilmResponse([]),
            'result' => new FilmResult([]),
        ...
        [$got, $err] = FilmMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FilmResult([])]);
        [, $err] = FilmMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FilmSpec([]), 'result' => null]);
        [, $err] = FilmMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FilmSpec(['step' => 's']),
            'result' => new FilmResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FilmOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FilmMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FilmSpec(['step' => 's']),
            'result' => new FilmResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FilmOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FilmMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FilmResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FilmSpec([]),
            'result' => new FilmResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FilmMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new FilmError('boom', 'boom');
        $utility->fetcher = function (FilmContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FilmSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FilmMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FilmContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FilmMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FilmResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FilmContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FilmSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FilmMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FilmResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FilmSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FilmMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FilmMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FilmResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FilmDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FilmResult(['ok' => false])]);
        $this->expectException(FilmError::class);
        FilmDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FilmResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FilmMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FilmBaseFeature();
        $b = new FilmBaseFeature();
        FilmFeatureAdd::call($ctx, $a);
        FilmFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FilmBaseFeature
    {
        $f = new FilmBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): FilmContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FilmSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new FilmResponse(['headers' => null]),
            'result' => new FilmResult([]),
        ]);
        FilmResultHeaders::call($ctx);
