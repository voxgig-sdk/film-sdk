...
The Ruby SDK for the Film API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = FilmSDK.test(testopts, sdkopts)
```
...
raise a `FilmError` (a `StandardError` subclass), so wrap
...
The main module (`Film_sdk`) exports the SDK class
