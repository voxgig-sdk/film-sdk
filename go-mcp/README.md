...
[MCP](https://modelcontextprotocol.io) server exposing the Film SDK as
two agent tools — `film_list` and `film_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/film-mcp)
make build

# 2. Provide credentials via the environment
export FILM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user film \
  -- /absolute/path/to/film-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./film-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// film_list: first page of records
{ "entity": "film" }
{ "entity": "film", "query": { } }

// film_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/film-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user film \
     -- "$PWD"/dist/*/film-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `film_list` and `film_load` tools now appear
   in new sessions. Ask the agent to *"list film using film"*
   and it calls `film_list` with `{"entity":"film"}`.
...
```sh
export FILM_APIKEY=sk_live_xxx            # API key
export FILM_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `film_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `film_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FILM_APIKEY` | API key sent with every request. |
| `FILM_BASE` | Optional override of the API base URL. |
...
```sh
./film-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"film_load","arguments":{"entity":"film","query":{"id":1}}}}'
```
